...
def draw_numbers(img, positions, numbers, show):
    for pos, num in zip(positions, numbers):
        if show:
            cv2.putText(img, str(num), pos, cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)
        else:
...
while cap.isOpened():
    ...
    if elapsed_time < display_time:
        ...
        if not game_over:
            cv2.putText(img, "Select the numbers in order", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            for i, num in enumerate(selected_numbers):
                cv2.putText(img, str(num), (50, 100 + i * 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        else:
            if sorted(selected_numbers) == correct_order:
                cv2.putText(img, "You win!", (width // 2 - 100, height // 2), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 3)
            else:
                cv2.putText(img, "Game over!", (width // 2 - 100, height // 2), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 3)
