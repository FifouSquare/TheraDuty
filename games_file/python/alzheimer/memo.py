    ...
    while True:
        ...
        if not has_won:
            for r in range(rows):
                for c in range(cols):
                    ...
                    cv2.rectangle(img, (GRID_BASE + top_left_x + GRID_INTERVAL,
                                        GRID_BASE + top_left_y + GRID_INTERVAL),
                                  (GRID_BASE + bottom_right_x,
                                   GRID_BASE + bottom_right_y),
                    ...
                    if i == 8:
                        ...
                        if line.shape[1] > 0:
                            ...
                            if length < 50 and not is_pinched:
                                ...
                                posX = ((
                                                cx - GRID_BASE - GRID_INTERVAL) // 75) // 2
                                posY = ((
                                                cy - GRID_BASE - GRID_INTERVAL) // 100) // 2
                                print(posX, posY)
                                has_to_draw = True
        ...
        if has_to_draw:
            if posY > rows - 1 or posX > cols - 1:
                ...
                if second_card == (-1, -1):
                    print("first_card != (posY, posX): ", first_card,
                          (posY, posX))
